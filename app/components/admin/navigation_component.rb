  ...
  class NavigationComponent < ApplicationViewComponent
    ...
    def navigation_items
      ...
      @navigation_items ||= begin
        ...
        items << { name: t('.locations'), path: admin_locations_path, icon: 'map-pin' } if current_user.admin_or_superadmin?

        # News Posts - admin and general
        items << { name: t('.news_posts'), path: admin_news_posts_path, icon: 'newspaper' } if current_user.admin_or_superadmin? || current_user.general?

        # RSS Feeds - admin and general
        items << { name: t('.rss_feeds'), path: admin_rss_feeds_path, icon: 'rss' } if current_user.admin_or_superadmin? || current_user.general?
